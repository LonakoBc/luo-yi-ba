...
export default function MultiplayerProducerGame({ room, self, producers, now, connection, send }) {
  ...
  const service = useMemo(() => createProducerGameService(producers), [producers]);
  const guessedIds = useMemo(() => new Set((self?.guesses ?? []).map((entry) => entry.producer.id)), [self]);
  ...
  return <div className="multiplayer-producer-game">
    ...
    <ProducerTable answer={answer} guesses={self?.guesses ?? []} hintLevel={room.hintLevel ?? 0} yearDebutRevealed={self?.yearDebutRevealed} finished={reveal} />
