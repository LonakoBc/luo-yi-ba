...
export async function generateDatabaseData({ catalogFile, databaseRoot, outputFile }) {
  ...
  for (const entry of catalog) {
    if (!/^[a-z0-9-]+$/u.test(entry.id ?? '') || !entry.name || !entry.file) throw new Error('歌姬目录项缺少合法的 id、name 或 file');
    ...
    if (!Array.isArray(rawSongs) || !rawSongs.length) throw new Error(`${entry.name}: 歌曲数据为空`);
    if (rawSongs.length !== entry.expectedSongCount) {
      throw new Error(`${entry.name}: 目录声明 ${entry.expectedSongCount} 首，实际 ${rawSongs.length} 首`);
    }

    const titles = new Set();
    const songs = rawSongs.map((song, index) => {
      const normalized = normalizeDatabaseSong(song, index, entry.name, `${entry.file} 第 ${index + 1} 条`);
      const titleKey = normalized.title.normalize('NFKC').toLocaleLowerCase('zh-CN');
      if (titles.has(titleKey)) throw new Error(`${entry.name}: 曲名重复：${normalized.title}`);
      titles.add(titleKey);
      return normalized;
    });
    libraries[entry.id] = songs;
    generatedCatalog.push({ id: entry.id, name: entry.name, songCount: songs.length });
  }
